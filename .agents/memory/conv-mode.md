...
**Half-open WS is the real "voice freezes until restart" cause — needs liveness on BOTH sides.** The browser AudioWorklet streams PCM continuously while the mic is open, so a live client never idles more than a couple seconds. A mobile network switch / backgrounding produces a half-open socket with NO close frame: `onclose`/`onerror` never fire client-side, and server `receive_bytes` never raises `WebSocketDisconnect`. **Symptoms:** mic sticks in listening/processing forever, reload doesn't help, all companions break at once (per-session), only a server restart clears it, HTTP push-to-talk stays fine. **Why the old code leaked:** `client_to_dg` looped forever on a 60s `receive_bytes` timeout that only sent Deepgram a KeepAlive and never broke → pump tasks never complete → `asyncio.wait(FIRST_COMPLETED)` never returns → `async with websockets.connect` never exits → Deepgram + ASGI WS pinned open indefinitely. **How to apply:** server — poll `receive_bytes` in short slices and TEAR DOWN after ~30s of no client audio (a live client can't be that quiet) + an absolute max-session cap; make ONE task the sole browser writer and have it emit `{"type":"ping"}` heartbeats (~10s) when Deepgram is quiet (avoids concurrent-send interleave). Client — track last-message time on every `ws.onmessage` and run a watchdog that resets to "off" (with a "tap to resume" message) if no server frame arrives for ~30s while active; never rely on `onclose`/`onerror` for half-open detection. **Gap left (uncommon):** a socket stuck in CONNECTING never fires `onopen`, so the watchdog (OPEN-only) won't catch a connect-stall — add a connect timeout if that surfaces.
