...
## Pre-3b checklist (from the Stage-3a whole-branch review — resolve before wiring `apply_candidate_durably` into the live path)

The repository merged clean (Opus review: ready-to-merge, no must-fix; inert). Fixed in 3a: the prod `PostgrestExecutor` now returns the RPC result (was `return=minimal`, unfaithful to the tests); multi + unknown durable round-trips are now proven end-to-end against real Postgres. These MUST be handled in 3b before the repository goes live:

- **MUST-FIX #1 — `subject_id` fragmentation vector.** `Candidate.subject_id` defaults to `"user"` (models.py) but the DB column + every slot index default to `"self"`. `apply_candidate_durably` trusts `candidate.subject_id`, so a candidate arriving with `"user"` while stored rows are `"self"` fetches no peers → inserts a SECOND active row under a different `subject_id`, silently bypassing `one_active_single`. Mitigated today only because the mapper sets `"self"`. Before 3b: either align `Candidate.subject_id`'s default to `"self"` (and update the `test_delta.py::_active_home` helper, which hardcodes `"user"`), or have the mapper/`apply_candidate_durably` normalize first-person subject identity. Pin it in code, not just convention.
- **Concurrency + connection caveats when wired:** `PsycopgExecutor` uses a single connection and requires autocommit — not safe for concurrent (`asyncio.gather`) use; add a docstring caveat. `_default_client_factory` opens a fresh `httpx.AsyncClient` per call (no pooling across a candidate's load+apply). Add small backoff/jitter between retries before concurrent load. When 3b lands, add one `asyncio.gather` durable test to exercise the **23505 insert-race** branch end-to-end (3a proved only the 40001 supersede race).
- **Config + robustness:** guard empty `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` (fail clearly at construction, not with an obscure httpx error at request time). Consider surfacing a malformed stored date rather than `_as_date` silently returning `None`.
- **Minor cleanup:** unused `_MIG` in `tests/test_repository_executor.py`; the `raise last_exc or ...` dead branch (only reachable at `max_retries<=0`).
